...
func Initialize() {
	host, ok := config.AppConfig.String("cache.hosts")
	if !ok {
		return
	}
	password, _ := config.AppConfig.String("cache.redis.password")
	tm := config.AppConfig.IntDefault("cache.redis.timeout.connect", 60*60)

	instance := NewRedisCache(host, password, time.Duration(tm))
	cache.RegisterCacheProvider(instance)
}

// until redigo supports sharding/clustering, only one host will be in hostList
func NewRedisCache(host string, password string, defaultExpiration time.Duration) RedisCache {
	var pool = &redis.Pool{
		MaxIdle:     config.AppConfig.IntDefault("cache.redis.maxidle", 5),
		MaxActive:   config.AppConfig.IntDefault("cache.redis.maxactive", 0),
		IdleTimeout: time.Duration(config.AppConfig.IntDefault("cache.redis.idletimeout", 240)) * time.Second,
		Dial: func() (redis.Conn, error) {
			protocol := config.AppConfig.StringDefault("cache.redis.protocol", "tcp")
			toc := time.Millisecond * time.Duration(config.AppConfig.IntDefault("cache.redis.timeout.connect", 10000))
			tor := time.Millisecond * time.Duration(config.AppConfig.IntDefault("cache.redis.timeout.read", 5000))
			tow := time.Millisecond * time.Duration(config.AppConfig.IntDefault("cache.redis.timeout.write", 5000))
